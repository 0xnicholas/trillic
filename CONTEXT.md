...
**产物包**:
v1 的交付单元,五件缺一不算交付:checkpoint + 溯源 model card + 对照报告 +
延迟档案 + 接入包。

**接入包**:
产物包第五件——让宿主照做即可接入的单元:drop-in 契约校验清单 + 宿主改动
patch(MODEL_ID / 缓存键 / 节省指标)+ 切换·观察·回滚 runbook + 非差异清单。
_Avoid_: 集成文档、集成指南(它包含可执行契约,不只是说明)

**供货**:
Trillic → refine 的关系定位:交付自包含产物(权重快照),不是代码依赖也
不是服务;再交付由触发器驱动(回归跌破 / 分布漂移 / 许可变化 / v2 立项),
无触发不交付。断供不影响 refine 运行与回滚。
_Avoid_: 上游依赖、集成

**替换**(接入语境):
只换 compress 阶段的 checkpoint 权重,不含 rewrite 阶段与任何宿主改造之外
的语义;判据持平(CI 下界不为负但无显著提升)= **不替换**。
_Avoid_: 接管、取代 refine
